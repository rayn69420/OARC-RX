...
TICKS_PER_DAY = 10*TICKS_PER_MINUTE
...
FORCE_HOURLY_BOT_BATTERY_BONUS = 0.000250
...
local function random_tenth(n)
    local n = n
    local ret = math.random(n*100-10, n*100+10)/100
    while ret < -1 or ret > 1 do
        ret = math.random(n*100-10, n*100+10)/100
    end
...
function modify_night_color(surface)
    local surface = surface or game.surfaces[GAME_SURFACE_NAME]
    local nc = surface.brightness_visual_weights
...
function FadeoutRenderOnTick()
    if (global.oarc_renders_fadeout and (#global.oarc_renders_fadeout > 0)) then
        for k, rid in pairs(global.oarc_renders_fadeout) do
            if (rid ~= nil) and rid.valid then
                local ttl = rid.time_to_live
                if ((ttl > 0) and (ttl < 100)) then
                    local color = rid.color
...
function ServerWriteFile(filename, msg)
    if not (global and global.ocfg) then return end
    if (global.ocfg.enable_server_write_files) then
        helpers.write_file(filename, msg, true, 0)
    end
end
...
function SafeTeleport(player, surface, target_pos)
    if (not player) or (not player.valid) or (not surface) or
    (not surface.valid) then
        return false
    end
    local safe_pos = surface.find_non_colliding_position("character",
    target_pos, 96, 0.5)
    if (not safe_pos) then
        return player.teleport(target_pos, surface)
    else
        return player.teleport(safe_pos, surface)
    end
end
...
function CreateGameSurface()
    ...
    if (GAME_SURFACE_NAME ~= "nauvis") then
        ...
        -- 0 (or 1) is noon
        -- 0.25 is dusk (surface.dusk) this is when solar begins to lose power
        -- 0.45 is evening (surface.evening) this is when solar goes completely out
        -- 0.5 should technically be midnight
...
function SetItemBlueprintTimeToLive(event)
    ...
    local type = built_entity.type
    if type == "entity-ghost" or type == "tile-ghost" then
        if global.ocfg.ghost_ttl ~= 0 then
            built_entity.time_to_live = global.ocfg.ghost_ttl
        end
    end
end
    ...
    for i = chunkArea.left_top.x, chunkArea.right_bottom.x, 1 do
        for j = chunkArea.left_top.y, chunkArea.right_bottom.y, 1 do
            ...
            if (distVar < tileRadSqr) then
                ...
                (script.active_mods["oarc-restricted-build"])) then
...
function Autofill(event)
    local eventEntity = event.entity or event.created_entity
