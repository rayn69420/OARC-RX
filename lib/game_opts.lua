...
function CreateGameOptionsTab(tab_container, player)
    ...
    local enemy_surface = (player and player.surface and player.surface.valid and player.surface.name) or GAME_SURFACE_NAME
    local enemy_text="Server Run Time: " .. formattime_hours_mins(game.tick) .. "\n" ..
    "Current Evolution: " .. string.format("%.4f", game.forces["enemy"].get_evolution_factor(enemy_surface)) .. "\n" ..
