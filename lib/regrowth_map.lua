...
function EnsureRegrowthGlobals()
    global.rg = global.rg or {}
    global.rg.map = global.rg.map or {}
    global.rg.removal_list = global.rg.removal_list or {}

    if (global.rg.force_removal_flag == nil) then
        global.rg.force_removal_flag = -2000
    end

    if (global.rg.timeout_ticks == nil) or (global.rg.timeout_ticks <= 0) then
        global.rg.timeout_ticks = REGROWTH_TIMEOUT_TICKS
    end

    if (global.rg.chunk_iter and (not global.rg.chunk_iter.valid)) then
        global.rg.chunk_iter = nil
    end

    if (global.rg.world_eater_iter and (not global.rg.world_eater_iter.valid)) then
...
function RegrowthMarkAreaForRemoval(pos, chunk_radius)
    ...
    for i=-chunk_radius,chunk_radius do
        ...
        if (global.rg.map[x] and (table_size(global.rg.map[x]) == 0)) then
            global.rg.map[x] = nil
        end
    end
end
...
function RefreshChunkTimer(pos, bonus_time)
    ...
    local c_timer = global.rg.map[c_pos.x][c_pos.y]
    if (c_timer and (c_timer >= 0)) then
...
function RefreshPlayerArea()
    ...
    local player_index = GetNextPlayerIndex()
...
function RegrowthSingleStepArray()
    ...
    if (not global.rg.chunk_iter or not global.rg.chunk_iter.valid) then
        global.rg.chunk_iter = surface.get_chunks()
    end
...
function OarcRegrowthRemoveAllChunks()
    ...
    for key,c_remove in pairs(global.rg.removal_list) do
        ...
        if (not global.rg.map[c_pos.x] or not global.rg.map[c_pos.x][c_pos.y]) then
            ...
            elseif (surface.get_pollution({c_pos.x*32,c_pos.y*32}) > 0) then
                global.rg.map[c_pos.x] = global.rg.map[c_pos.x] or {}
                global.rg.map[c_pos.x][c_pos.y] = game.tick
...
function WorldEaterSingleStep()
    ...
    if (not global.rg.world_eater_iter or not global.rg.world_eater_iter.valid) then
        global.rg.world_eater_iter = surface.get_chunks()
    end
    ...
    local entities = surface.find_entities_filtered{area=next_chunk.area,
    ...
    entities = surface.find_entities_filtered{area=next_chunk.area,
    ...
    if (c_timer == -1) then
        ...
        local entities = surface.find_entities_filtered{area=area, force={"enemy", "neutral"}, invert=true}
