...
local function buildTree(recipe, count, indent)
    local str = str
...
function(command)
    ...
    if command.parameter then
        if prototypes.recipe[command.parameter] then
            player.print("[font=debug-mono]"..buildTree(prototypes.recipe[command.parameter]).."[/font]")
        ...
        for _, item in pairs(prototypes.recipe) do
            helpers.write_file("tree_all.txt", buildTree(item), true, player.index)
        end
    end
end)

commands.add_command("test", "test chunk cleanup", tools.cleanup_chunks)
...
local function get_assemblers(item, per_sec, assembler)
    if not prototypes.item[item] then return end
    if not prototypes.entity[assembler] then return end
    if not prototypes.recipe[item] then return end
    local assembler = prototypes.entity[assembler]
...
commands.add_command("give", "Share some of your coin with another player\nUsage: /give zerocool 10000", function(command)
    ...
    if game.players[args[1]] and global.markets[args[1]] then
        ...
        if global.markets[player.name].balance >= amount then
            ...
            global.markets[args[1]].balance = global.markets[args[1]].balance + amount
...
function(command)
    ...
    p.set_quick_bar_slot(62, "storage-chest");
    p.set_quick_bar_slot(63, "passive-provider-chest");
    p.set_quick_bar_slot(64, "requester-chest");
    p.set_quick_bar_slot(65, "buffer-chest");
    
    p.set_quick_bar_slot(66, "active-provider-chest");
...
-- commands.add_command("join", "join another player's base (you will lose your ability to sell items)", function(command)
--     local player = game.players[command.player_index]
--     if not command.parameter then
--         player.print("You forgot the player's name you'd wish to join")
--         return
--     end
--     if not game.players[command.parameter] then
--         player.print("Make sure you've spelled the player's name correctly. Use tab to autoenter.")
--         return
--     end
--     if game.players[command.parameter] then
--         if not game.players[command.parameter].connected then
