...
function BuildSiloAttempt(event)
    ...
    -- Validation
    if (built_entity == nil) or (not built_entity.valid) then return end

    local e_name = built_entity.name
    if (built_entity.name == "entity-ghost") then
        e_name = built_entity.ghost_name
    end

    if (e_name ~= "rocket-silo") then return end

    -- Check if it's in the right area.
    local epos = built_entity.position
    ...
    if (built_entity.last_user ~= nil) then
        FlyingText("Can't build silo here! Check the map!", epos, my_color_red, built_entity.surface)
        if (built_entity.name == "entity-ghost") then
            built_entity.destroy()
        else
            built_entity.last_user.mine_entity(built_entity, true)
        end
    else
