...
local function get_force_item_statistics(force, surface)
    return force.get_item_production_statistics(surface)
end

local function get_force_fluid_statistics(force, surface)
...
function SetNeutralForceAllowedRecipes()
    ...
    for i, v in pairs(NEUTRAL_FORCE_RECIPES) do
        local recipe = game.forces["neutral"].recipes[i]
        if recipe then
            recipe.enabled = true
        end
    end
...
function EnsureMagicFactoryGlobals()
    global.omagic = global.omagic or {}
    if (global.omagic.building_total_count == nil) then
        global.omagic.building_total_count = 0
    end
    global.omagic.factory_positions = global.omagic.factory_positions or {}
    global.omagic.furnaces = global.omagic.furnaces or {}
    global.omagic.chemplants = global.omagic.chemplants or {}
    global.omagic.refineries = global.omagic.refineries or {}
    global.omagic.assemblers = global.omagic.assemblers or {}
    global.omagic.centrifuges = global.omagic.centrifuges or {}
end
...
function RequestSpawnSpecialChunk(player, spawn_function, feature_name)
    if not player or not player.valid or not player.character or
    not player.character.valid then
        return false
    end
    local closest_chunk = FindClosestMagicChunk(player)
    if not closest_chunk then return false end
    local player_chunk = GetChunkPosFromTilePos(player.character.position)
    if ((closest_chunk.x == player_chunk.x) and
...
function SpawnOilRefineryChunk(chunk_pos)
    ...
    -- 2x Refineries
    table.insert(oil_chunk.refineries, SpawnMagicBuilding("oil-refinery", {
        x = center_pos.x - 5,
        y = center_pos.y - 8
    }))
    table.insert(oil_chunk.refineries, SpawnMagicBuilding("oil-refinery", {
    ...
    -- 6x Chem Plants
    table.insert(oil_chunk.chemplants, SpawnMagicBuilding("chemical-plant", {
        x = center_pos.x - 10,
        y = center_pos.y + 8
    }))
    table.insert(oil_chunk.chemplants, SpawnMagicBuilding("chemical-plant", {
...
function SpawnCentrifugeChunk(chunk_pos)
    ...
    center_pos = GetCenterTilePosFromChunkPos(chunk_pos)
    local centrifuge_chunk = {
        ["energy_input"] = spawnSpecialChunkInputElec(center_pos),
        ["entities"] = {}
    }
    
    -- 1 Centrifuge (MORE THAN ENOUGH!)
    table.insert(centrifuge_chunk.entities, SpawnMagicBuilding("centrifuge", {
        x = center_pos.x,
        y = center_pos.y - 10
    }))
...
function SpawnMagicBuilding(entity_name, position)
    ...
    local magic_building = game.surfaces[GAME_SURFACE_NAME].create_entity {
        name = entity_name,
        position = position,
        force = "neutral",
        direction = direction
...
function MagicFurnaceOnTick()
    ...
    for entry_idx, entry in pairs(global.omagic.furnaces) do
        ...
        for idx, furnace in pairs(entry.entities) do
            ...
            if not input_item_name then goto next_furnace end
            ...
            if (#recipe.products ~= 1) or (#recipe.ingredients ~= 1) then
                log(
                "MagicFurnaceOnTick - Recipe product/ingredient more than 1?")
                goto next_furnace
            end
            ...
            if (not furnace.last_user) then
                ...
                if (player_entities and player_entities[1] and
                player_entities[1].last_user) then
            ...
            input_inv.remove({
                ...
                count = recipe_count * recipe_ingredient.amount
            })
            output_inv.insert({
                name = recipe_product.name,
                count = recipe_count * recipe_product.amount
            })
            ...
            if (furnace.last_user) then
                get_force_item_statistics(furnace.last_user.force,
                furnace.surface).on_flow(
                recipe_ingredient.name,
                -recipe_count * recipe_ingredient.amount)
                get_force_item_statistics(furnace.last_user.force,
                furnace.surface).on_flow(
                recipe_product.name, recipe_count * recipe_product.amount)
            end
        ...
        for idx, chemplant in pairs(entry.chemplants) do
            ...
            for _, v in ipairs(recipe.ingredients) do
                if (not input_items[v.name] or (input_items[v.name] < v.amount)) then
                    if (not input_fluids[v.name] or
                    (input_fluids[v.name] < v.amount)) then
            ...
            if recipe_product.type == "fluid" then
                ...
                if ((chemplant.get_fluid_count(recipe_product.name) +
                recipe_product.amount) > 100) then
                    goto next_chemplant -- Not enough space for ouput
                end
                
                
                chemplant.insert_fluid({
                    name = recipe_product.name,
                    amount = recipe_product.amount
                })
                if (chemplant.last_user) then
                    get_force_fluid_statistics(chemplant.last_user.force,
                    chemplant.surface)
                ...
                if not output_inv.can_insert({
                    name = recipe_product.name,
                    amount = recipe_product.amount
                }) then goto next_chemplant end
                
                
                -- Add recipe count to output
                output_inv.insert({
                    name = recipe_product.name,
                    count = recipe_product.amount
                })
                if (chemplant.last_user) then
                    get_force_item_statistics(chemplant.last_user.force,
                    chemplant.surface)
                    .on_flow(recipe_product.name, recipe_product.amount)
                end
            end
    ...
    for entry_idx, entry in pairs(global.omagic.refineries) do
        ...
        for idx, refinery in pairs(entry.refineries) do
            ...
            if (recipe.name == "advanced-oil-processing") then
                ...
                if ((not refinery.fluidbox[1]) or
                (refinery.fluidbox[1].amount < 50)) then
                    goto next_refinery
                end -- Not enough water
                if ((not refinery.fluidbox[2]) or
                (refinery.fluidbox[2].amount < 100)) then
                    goto next_refinery
                end -- Not enough crude       
                
                if ((refinery.fluidbox[3]) and
                (refinery.fluidbox[3].amount > 25)) then
                    goto next_refinery
                end -- Not enough space for heavy
                if ((refinery.fluidbox[4]) and
                (refinery.fluidbox[4].amount > 45)) then
                    goto next_refinery
                end -- Not enough space for light
                if ((refinery.fluidbox[5]) and
                (refinery.fluidbox[5].amount > 55)) then
                    goto next_refinery
                end -- Not enough space for petro
                ...
                if (refinery.last_user) then
                    get_force_fluid_statistics(refinery.last_user.force,
                    refinery.surface)
                    .on_flow("water", -50)
                    get_force_fluid_statistics(refinery.last_user.force,
                    refinery.surface)
            ...
            elseif (recipe.name == "basic-oil-processing") then
                ...
                if ((not refinery.fluidbox[1]) or
                (refinery.fluidbox[1].amount < 100)) then
                    goto next_refinery
                end -- Not enough crude
                if ((refinery.fluidbox[2]) and
                (refinery.fluidbox[2].amount > 45)) then
                    goto next_refinery
                end -- Not enough space for petro
    ...
    for entry_idx, entry in pairs(global.omagic.assemblers) do
        ...
        for idx, assembler in pairs(entry.entities) do
            ...
            if (assembler.last_user) then
                get_force_item_statistics(assembler.last_user.force,
                assembler.surface).on_flow(
                recipe_product.name, recipe_product.amount)
            end
            ...
            for _, v in ipairs(recipe.ingredients) do
                ...
                elseif (input_fluids[v.name]) then
                    ...
                    if (assembler.last_user) then
                        get_force_fluid_statistics(assembler.last_user.force,
                        assembler.surface)
                        .on_flow(v.name, -v.amount)
                    end
                end
        ...
        for idx, centrifuge in pairs(entry.entities) do
            ...
            if (recipe.name == "uranium-processing") then
                ...
                if (centrifuge.last_user) then
                    get_force_item_statistics(centrifuge.last_user.force,
                    centrifuge.surface)
                ...
                for _, v in ipairs(recipe.ingredients) do
                    if (input_items[v.name]) then
                        input_inv.remove({name = v.name, count = v.amount})
                        if (centrifuge.last_user) then
                            get_force_item_statistics(
                            centrifuge.last_user.force, centrifuge.surface)
                            .on_flow(v.name,
                            -v.amount)
                        end
                    end
...
function CoinsFromEnemiesOnPostEntityDied(event)
    if (not event.entity or not prototypes.entity[event.entity.name]) then
        return
    end
    if event.cause then
        ...
        if (coin_chance) then
            ...
            elseif event.cause.name == "gun-turret" and event.cause.last_user then
...
function DropCoins(count, player)
    ...
    if (count < 1) then
        if (math.random() < count) then drop_amount = 1 end
