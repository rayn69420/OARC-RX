...
function InitOarcConfig()
    ...
    global.ocfg = global.ocfg or {}
    global.ocfg.debug = DEBUG_MODE

    
    global.ocore = global.ocore or {}
    global.ocore.playerSpawns = global.ocore.playerSpawns or {}
    global.ocore.uniqueSpawns = global.ocore.uniqueSpawns or {}
    global.ocore.sharedSpawns = global.ocore.sharedSpawns or {}
    global.ocore.playerCooldowns = global.ocore.playerCooldowns or {}
    global.ocore.waitingBuddies = global.ocore.waitingBuddies or {}
    global.ocore.delayedSpawns = global.ocore.delayedSpawns or {}
    global.ocore.buddySpawnOpts = global.ocore.buddySpawnOpts or {}
    global.ocore.buddyPairs = global.ocore.buddyPairs or {}
    global.oarc_renders_fadeout = global.oarc_renders_fadeout or {}
    global.siloPosition = global.siloPosition or {}
    global.markets = global.markets or {}
    global.config = global.config or {}
    global.config.decon_triggers = global.config.decon_triggers or {
        decon_area = true,
        built_entity = true,
        mined_entity = true,
        fired_rocket = true,
    ...
    if global.config.limit_turret_upgrades == nil then
        global.config.limit_turret_upgrades = false
    end
    
    if (script.active_mods["clean-tutorial-grid"]) then
...
function EnsureOarcConfig()
    if not global.ocfg then
        InitOarcConfig()
    end
end
