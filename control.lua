...
script.on_event(defines.events.on_gui_switch_state_changed, function(event)
    ...
    if player_market then
        if player_market.followers_switch and event.element ==
        player_market.followers_switch then
...
script.on_event(defines.events.on_gui_click, function(event)
    ...
    if global.ocfg and global.ocfg.enable_coin_shop then
        if element_name == "market_button" or
        element_name == "stats_button" then
            market.ensure_player_gui(player)
        end
        if element_name == "market_admin_fund_button" then
    ...
    local player_market = market.get_market_view(player)
    if player_market then
        if element_name == "market_button" then
            market.toggle_market_gui(player)
        end
        if element_name == "stats_button" then
            market.toggle_stats_gui(player)
        end
        if element_name == "market_admin_fund_button" then
            market.admin_grant_self(player)
        end
        if player_market.item_buttons and
        player_market.item_buttons[element_name] then
            local button =
            player_market.item_buttons[element_name]
            local click = event.button
            local shift = event.shift
            local ctrl = event.control
            market.purchase(player, button.name, click, shift, ctrl)
        end
        if player_market.upgrade_buttons and
        player_market.upgrade_buttons[element_name] then
            local button =
            player_market.upgrade_buttons[element_name]
            market.upgrade(player, button.name)
        end
        if global.ocfg.enable_groups == true then
            if player_market.follower_buttons and player_market.follower_buttons[element_name] then
                local button =
                player_market.follower_buttons[element_name]
        ...
        if global.ocfg.enable_shared_purchasing == true then
            if player_market.shared_buttons and player_market.shared_buttons[element_name] then
                local button =
                player_market.shared_buttons[element_name]
                if market.shared_func_table[button.name](player) then
                    market.upgrade_shared(player, button.name)
                end
            end
        end
        if player_market.special_buttons and player_market.special_buttons[element_name] then
            local button =
            player_market.special_buttons[element_name]
            if market.special_func_table[button.name](player) then
                if button.name == "special_offshore-pump" then
                    market.withdraw(player, player_market.stats.waterfill_cost)
                    return
                else
...
script.on_event(defines.events.on_player_ammo_inventory_changed, function(event)
    ...
    if not ammo_inv then return end
    ...
    local target_pos = player.shooting_state and player.shooting_state.position
    if not target_pos then return end
    if item.name == "rocket" and global.config.decon_triggers.fired_rocket then
        tools.add_shoot_log(tools.get_secs() .. "," .. player.name .. ",shot-rocket," .. tools.pos_tostring(player.position) .. "," .. tools.pos_tostring(target_pos))
    elseif item.name == "explosive-rocket" and global.config.decon_triggers.fired_explosive_rocket then
        tools.add_shoot_log(tools.get_secs() .. "," .. player.name .. ",shot-explosive-rocket," .. tools.pos_tostring(player.position) .. "," .. tools.pos_tostring(target_pos))
    elseif item.name == "atomic-bomb" and global.config.decon_triggers.fired_nuke then
        tools.add_shoot_log(tools.get_secs() .. "," .. player.name .. ",shot-nuke," .. tools.pos_tostring(player.position) .. "," .. tools.pos_tostring(target_pos))
    end
end)
...
script.on_event(defines.events.on_player_created, function(event)
    ...
    -- as a fresh join and wiping the player's market/spawn data.
    if has_existing_spawn or has_existing_market then
        log("Reusing existing player state during on_player_created for " ..
            player.name)
...
script.on_event(defines.events.on_player_left_game, function(event)
    ...
    if (player and
    (global.ocfg.minimum_online_time > 0) and
    (player.online_time < (global.ocfg.minimum_online_time * TICKS_PER_MINUTE))) then
        -- Local/solo save testing should not wipe the only player's base on exit.
        if (total_players <= 1) then
            log("Skipping early-leave cleanup for solo save: " .. player.name)
            return
        end
        if not FindPlayerSharedSpawn(player.name) then
...
script.on_event(defines.events.on_built_entity, function(event)
    ...
    if not (built_entity and built_entity.valid) then return end
    local player = game.players[event.player_index]
    
    if global.ocfg.enable_regrowth then
        if (built_entity.surface.name ~= GAME_SURFACE_NAME) then
            return
        end
        RegrowthMarkAreaSafeGivenTilePos(built_entity.position, 2, false)
    end
...
script.on_event(defines.events.on_gui_closed, function(event)
    ...
    if event.element then
        local element_name = event.element.name
        local player_market = player and market.get_market_view(player)
        if player_market and ((player_market.market_frame and
        event.element == player_market.market_frame) or
        (element_name == "oarc_market_frame")) then
            market.close_market_gui(player)
        elseif player_market and ((player_market.stats_frame and
        event.element == player_market.stats_frame) or
        (element_name == "oarc_stats_frame")) then
