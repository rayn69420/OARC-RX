...
function personal_storage.store(player, item)
    ...
    if inv_item_count == prototypes.item[item].stack_size then
        return storage[item]
    end
    if inv_item_count < prototypes.item[item].stack_size and storage[item] ==
        0 then return 0 end
    if inv_item_count < prototypes.item[item].stack_size and storage[item] >=
        0 then
        local amount_to_withdraw = prototypes.item[item].stack_size -
                                       inv_item_count
