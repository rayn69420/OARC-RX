...
function tools.add_decon_log(data)
    helpers.write_file(tools.decon_filepath, data .. "\n", true, 0) -- write data
end
function tools.add_shoot_log(data)
    helpers.write_file(tools.shoot_filepath, data .. "\n", true, 0) -- write data
end
...
local function get_statistics_surface()
    if GAME_SURFACE_NAME and game.surfaces[GAME_SURFACE_NAME] then
        return game.surfaces[GAME_SURFACE_NAME]
    end
    return game.surfaces["nauvis"] or game.surfaces[1]
end

local function get_item_production_statistics(force)
    local surface = get_statistics_surface()
...
local function get_item_all_time(force, item)
    local stats = get_item_production_statistics(force)
    if not stats then return 0 end
    return stats.get_output_count(item)
end
...
function tools.statistics_log(event)
    if event.tick > 10 then
        ...
        if global.highest_spm.amount > old_highest then
            ...
            helpers.write_file("statistics/SPM.txt",
...
function tools.shared_log(event)
    if event.tick > 10 then
        helpers.write_file(
        "statistics/shared.log",
        "Hour " .. tools.round(game.tick/(60*60*60), 0) .. "\n\n"
        .. "===========Shared Items==========\n\n"
        .. helpers.table_to_json(global.oshared.items) .. "\n\n"
        .. "===========Shared Power==========\n\n"
...
function tools.get_player(o) -- pass in table, string, or int
    ...
    if o_type == 'table' or o_type == 'userdata' then -- if its already a player-like object
        p = o -- just keep it
...
--         if global.material_chest and global.material_chest.valid then
--             local chest_inv = global.material_chest.get_inventory(defines.inventory
--             .chest)
--             chest_inv.clear()
--             local list = game.surfaces[GAME_SURFACE_NAME]
--             .find_entities_filtered {
--                 type = "entity-ghost",
--                 force = global.material_chest.force
--             }
...
function tools.replace(player, e1, e2)
    ...
    if prototypes.entity[e1] or prototypes.tile[e1] then
        ...
        elseif prototypes.tile[e1] then
...
function tools.cleanup_chunks()
    ...
    for chunk in surface.get_chunks() do
        for _, position in pairs(get_surrounding_positions({chunk.x, chunk.y}, pollution_chunks)) do
            if get_pollution(get_center_of_chunk_position(position)) ~= 0 then
                return
            end
        end
